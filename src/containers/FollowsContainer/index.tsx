...
const FollowsContainer = () => {
    ...
    const getData = async () => {
        if (user && user.username && user.username.length>0) {
            const res = await fetch('/api/user/get-follows/'+user.username)
            if (res.status === 200) {
    ...
    return (
        ...
        >
            <PageTitle
                formatId='page_title_following'
            />
            <div
                className='follows-container__list'
            >
                {
                    (data && data.length>0)
                    ?
                    ...
                    :
                    <div
                        className='follows-container__list__message'
                    >
                        <span
                            className="title"
                        >
