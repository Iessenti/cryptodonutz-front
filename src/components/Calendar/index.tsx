...
const Calendar = (props: {
    ...
    const handleClick = (event: any) => {
        if ( event.target.className.includes('calendar-wrapper__calendar__main__active') ) {
            if (date.start === '') {
                console.log(1)
    ...
    useLayoutEffect(() => {
      ...
      document?.getElementById('calendar')?.addEventListener('click', handleClick)
    });
    ...
    function createCalendar(year: any, month: any): any {
        ...
        while (d.getMonth() === mon) {
            if ( 
                (d.getDate().toString().length===1 ? '0'+d.getDate().toString() : d.getDate().toString()) === date.start
                ||
                (d.getDate().toString().length===1 ? '0'+d.getDate().toString() : d.getDate().toString()) === date.end
                ||
                (
                    date.end.length!==0 && date.start.length!==0
                    ?
                    ( parseInt((d.getDate().toString().length===1 ? '0'+d.getDate().toString() : d.getDate().toString())) > parseInt(date.start) ) && ( parseInt((d.getDate().toString().length===1 ? '0'+d.getDate().toString() : d.getDate().toString())) < parseInt(date.end) )
                    :
                    false
                )
            ) {
                table += `<td class="choosen calendar-wrapper__calendar__main__active">${d.getDate().toString().length===1 ? '0'+d.getDate() : d.getDate()}</td>`
            } else {
                table += `<td class="calendar-wrapper__calendar__main__active">${d.getDate().toString().length===1 ? '0'+d.getDate() : d.getDate()}</td>`
            }
    ...
    useEffect( () => {
        console.log('no')
        setData(createCalendar( parseInt(new Date().toISOString().slice(0,4) ), date.month))
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [date])

    function getDay(date: any) { // получить номер дня недели, от 0 (пн) до 6 (вс)
        let day = date.getDay();
